...
import { LikeButton } from '../../src/components/publicacion/LikeButton'
...
/**
 * Test de componente: `LikeButton` deshabilitado en publicación propia.
 * Fuente de verdad: specs/001-user-interactions/tasks.md (T029) y
 * specs/001-user-interactions/spec.md (FR-003).
 */
...
function renderizarLikeButton(publicacion: Publicacion) {
  return render(
    <MemoryRouter>
...
describe('LikeButton', () => {
  it('está deshabilitado cuando la publicación es propia del usuario autenticado', () => {
    const autor = crearUsuario({ id: 'autor-1' })
    mockearAuth(autor)
    const publicacion = crearPublicacion({ autorId: 'autor-1' })

    renderizarLikeButton(publicacion)

    expect(screen.getByRole('button')).toBeDisabled()
  })

  it('no está deshabilitado cuando la publicación no es propia', () => {
